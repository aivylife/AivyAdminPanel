<template>
  <q-page padding>
    <div class="row justify-between items-center q-mb-lg">
      <div class="text-h4">{{ isEdit ? 'Редактировать' : 'Создать' }} модуль</div>
      <q-btn
        flat
        color="grey-8"
        icon="arrow_back"
            ...
            <q-list v-if="groupedDayExercises.length > 0" bordered class="q-mb-md">
              ...
              >
                 <div class="row justify-between items-center q-pa-sm">
                   <span class="text-grey">День {{ dayGroup.dayNumber }}</span> <!-- Просто отображаем номер дня -->
                   <div>
                     <q-btn
                 ...
                 <q-list v-if="dayGroup.assignments.length > 0" separator dense class="q-mt-sm exercise-list">
                    <q-item v-for="(assignment) in dayGroup.assignments" :key="assignment.exerciseId + '-' + assignment.order" class="exercise-item">
                      <!-- Используем getExerciseDetailsById для получения названия -->
                     <q-item-section>
                        <q-item-label>({{ assignment.order }}) {{ assignment.exercise?.title || 'Упражнение не найдено' }}</q-item-label>
                      </q-item-section>
...
const columns = [
  ...
  { name: 'actions', label: 'Действия', align: 'center' as const, field: 'actions' }
];
...
const fetchMarathons = async () => {
  ...
  try {
    const response = await api.get<{ data: { id: number; title: string }[] }>('/api/marathon');
...
const fetchAllExercises = async () => {
   try {
    const response = await api.get<{ data: Exercise[] }>('/api/exercise');
    if (response.data && Array.isArray(response.data.data)) {
...
const fetchModuleData = async (id: string | number) => {
    loading.value = true;
    try {
      const response = await api.get(`/api/module/${id}`, {params: {relations: 'dayExercises,dayExercises.exercise,marathon'}});
      const moduleData = response.data;
      // Сортируем полученные назначения по дню и порядку
      const sortedDayExercises = (moduleData.dayExercises || []).sort(
         (a: { dayNumber: number; order: number; exerciseId: number }, b: { dayNumber: number; order: number; exerciseId: number }) => {
    ...
    const groupedDayExercises = computed(() => {
      ...
      return Object.entries(groups)
               .map(([day, assignments]) => ({ dayNumber: parseInt(day, 10), assignments }))
    ...
    const recalculateDayNumbers = () => {
      ...
      const allDayNumbers = [...new Set(formData.value.dayExercises.map(a => a.dayNumber))].sort((a, b) => a - b);
      ...
      formData.value.dayExercises = formData.value.dayExercises.map(assignment => ({
        ...assignment,
    ...
    const addDay = () => {
      ...
      const maxDayNumber = formData.value.dayExercises.length > 0
    ...
    const removeDayAssignments = (dayNumber: number) => {
      ...
      formData.value.duration = Math.max(...formData.value.dayExercises.map(d => d.dayNumber), 0);
    };
    ...
    const updateSelectedExercisesForDay = async (selectedIds: number[]) => {
      ...
      if (newIds.length > 0) {
        ...
        formData.value.dayExercises = formData.value.dayExercises.filter(
          assignment => !(assignment.dayNumber === dayNum && assignment.exerciseId === -1)
        );
      }
    ...
    const removeExerciseAssignment = (assignmentToRemove: DayExerciseAssignment) => {
        // Удаляем только конкретное упражнение
        formData.value.dayExercises = formData.value.dayExercises.filter(assignment =>
    ...
    const isDayFirst = (dayNumber: number): boolean => {
      const allDayNumbers = [...new Set(formData.value.dayExercises.map(a => a.dayNumber))];
    ...
    const getExerciseDetailsById = (exerciseId: number): Exercise | undefined => {
      ...
      if (exerciseId === -1) {
        return { id: -1, title: 'Нажмите "Добавить упражнения"', type: 'info' } as Exercise;
      }
      return allExercises.value.find(ex => ex.id === exerciseId);
    };

    // Наблюдатель за изменением длительности
    watch(() => formData.value.duration, (newDuration, oldDuration) => {
      // Получаем все уникальные номера дней
      const existingDayNumbers = new Set(formData.value.dayExercises.map(a => a.dayNumber));
  ...
  showMarathonSelector.value = false;
    ...
    const onSubmit = async () => {
      ...
      try {
        ...
        Object.keys(assignmentsByDay).sort((a, b) => parseInt(a, 10) - parseInt(b, 10)).forEach(dayNumStr => {
        ...
        if (isEdit.value) {
          await api.patch(`/api/module/${route.params.id}`, dataToSave);
          $q.notify({
            type: 'positive',
            message: 'Модуль успешно обновлен'
          });
        } else {
          await api.post('/api/module', dataToSave);
          $q.notify({
            type: 'positive',
        ...
        const message = error.response?.data?.message || 'Ошибка при сохранении модуля';
        $q.notify({
          type: 'negative',
          message: message
        });
    ...
    const isExerciseFirst = (assignment: DayExerciseAssignment): boolean => {
      const dayExercises = formData.value.dayExercises.filter(
        ex => ex.dayNumber === assignment.dayNumber
    ...
    const moveExerciseUp = (assignment: DayExerciseAssignment) => {
      ...
      const currentIndex = dayExercises.findIndex(ex => ex.order === assignment.order);
    ...
    const swapExerciseOrders = (exercise1: DayExerciseAssignment, exercise2: DayExerciseAssignment) => {
      ...
      $q.notify({
        type: 'info',
        message: `Порядок упражнений изменен: ${exercise1.exercise?.title || 'Упражнение'} ⇄ ${exercise2.exercise?.title || 'Упражнение'}`
      });
    };
