<template>
  <div>
    <div class="text-h5 q-mb-lg">{{ isEdit ? 'Редактировать' : 'Создать' }} модуль</div>
...
watch(() => props.modelValue, async (newValue) => {
  if (newValue) {
    try {
      const response = await api.get(`/api/module/${newValue.id}`, {
        params: { relations: 'dayExercises,dayExercises.exercise' }
      });
      const moduleData = response.data;
...
const sortExercises = (dayNumber: number) => {
  const dayExercises = formData.value.dayExercises.filter(de => de.dayNumber === dayNumber);
  ...
  formData.value.dayExercises = formData.value.dayExercises.map(de => {
    if (de.dayNumber === dayNumber) {
      const index = dayExercises.findIndex(e => e.exerciseId === de.exerciseId);
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (formData.value.id) {
      await api.patch(`/api/module/${formData.value.id}`, moduleData);
      $q.notify({
        type: 'positive',
        message: 'Модуль успешно обновлен'
      });
    } else {
      await api.post('/api/module', moduleData);
      $q.notify({
        type: 'positive',
