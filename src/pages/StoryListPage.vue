<template>
  <q-page padding>
    <div class="row justify-between items-center q-mb-lg">
      <div class="text-h4">Сторис</div>
      <q-btn
        label="Создать сторис"
        color="primary"
        icon="add"
        @click="router.push('/stories/new')"
        class="q-px-md"
      />
    </div>
    ...
    <q-card>
      <q-card-section class="row justify-between items-center">
        <div class="text-h6">Список сторис</div>
      </q-card-section>
      <q-separator />
      <q-card-section>
        <q-table
          ...
          @request="onRequest"
          @row-click="(evt, row) => editStory(row.id)"
        >
            ...
            <q-td :props="props" class="q-gutter-sm">
              <div class="row no-wrap items-center flex-start">
                <q-btn
                  ...
                  color="primary"
                  icon="edit"
                  @click.stop="editStory(props.row.id)"
                >
                ...
                <q-btn
                  ...
                  color="negative"
                  icon="delete"
                  @click.stop="confirmDeleteStory(props.row.id)"
                >
          ...
          <template v-slot:no-data>
            ...
            >
              ...
              <span>Нет доступных сторис</span>
              <q-btn
                label="Создать первый сторис"
                color="primary"
                no-caps
                @click="router.push('/stories/new')"
                class="q-ml-md"
...
const onRequest = async (props: { pagination: QTableProps['pagination'] }) => {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: 'Ошибка при загрузке сторис' })
  } finally {
    loading.value = false
  }
}
...
const confirmDeleteStory = (id: number) => {
  $q.dialog({
    title: 'Подтверждение',
    message: 'Вы действительно хотите удалить этот сторис?',
    cancel: true,
    persistent: true,
  }).onOk(async () => {
    try {
